...
def makeText(text, color, bgcolor, top, left, size=None):
    if size is None:
...
def puzzle(sreen):
    ...
    DISPLAYSURF = sreen
    pygame.init()
    original_images = [pygame.image.load(f'images/sobor/{i}.png') for i in range(1, 25)]
    ...
    RESET_SURF, RESET_RECT = makeText(
        'Reset', TEXT, BGCOLOR, win_width - 130, win_height - 530, size=25)
    NEW_SURF, NEW_RECT = makeText(
        'New Game', TEXT, BGCOLOR, win_width - 130, win_height - 500, size=25)
